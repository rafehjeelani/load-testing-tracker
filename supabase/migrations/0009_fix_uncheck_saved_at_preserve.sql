-- Fixes a bug in 0008: unchecking a step's outcome while a comment or
-- evidence was still attached fell through to the generic "outcome changed"
-- branch, which re-stamped saved_at to now() -- re-stamping forward in time
-- instead of leaving the original time alone as intended (and as the
-- candidate-facing confirmation message promises). Only the true "nothing
-- else recorded either" case should touch saved_at when clearing to null;
...
begin
  ...
  if p_outcome is null then
    -- v_saved_at already holds the previously-stored value (from the select
    -- above); only overwrite it to null when nothing else is left recorded.
    -- Otherwise it's left untouched -- a comment or evidence is still
    -- attached, so the original time stays meaningful.
    if (p_comment is null or length(trim(p_comment)) = 0)
