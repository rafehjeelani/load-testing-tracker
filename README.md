...
- **Reference screenshots on every step**: the candidate wizard (and the Network Check gate) now shows a small "Reference: what to capture" panel -- a thumbnail (click to open full size) plus the exact guidance text from the Screenshot Evidence Guide (e.g. "Completed speed-test page with download and upload speeds visible") -- so candidates can match what they're looking at against what's expected before uploading evidence. `src/lib/referenceScreenshots.ts` keys these by the step's exact name, matching `STANDARD_STEPS`; a step that's been renamed or customized just won't show a reference image, which is expected. The wizard's intro text also now points candidates to log errors, reconnecting screens, or long delays under Disconnection instead of uploading them as step evidence. One source image (Secondary Device Connected) had a Teams notification exposing real email addresses and what looked like a plaintext password -- cropped out before use, since this page is public and unauthenticated.
