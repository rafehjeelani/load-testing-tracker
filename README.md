...
### A disconnection no longer resets every step to blank

On the real assessment platform, a disconnection doesn't always repeat every step -- sometimes it auto-reconnects and nothing repeats, sometimes a refresh skips some steps (consent, onboarding) but not others, and which ones vary by disconnect. There's no fixed list to hardcode. So instead of wiping every step back to "Not yet reported" on a disconnection (forcing the candidate to re-pick an outcome for steps nothing actually happened to, just to unlock Next again), each step's "current" value is now its most recent answer across every attempt -- an untouched step keeps showing exactly what it showed before the disconnection, comment and evidence included, and the wizard's Next button no longer blocks on it. A step the candidate (or staff) does touch again still writes a genuinely new, separately-timestamped row under the new attempt, exactly as before -- so the Session Log and Report Session Timeline are unaffected, still showing only real re-submissions, never a fake "resubmitted" event for a step nothing happened to.

`0017_carry_forward_current_status.sql` changes `rpc_get_candidate_state`'s `step_reports` (the candidate's own "what to answer now" view) and `rpc_edit_step_saved_at` (correcting a step's time) to read each step's latest row across all attempts instead of strictly the current attempt's row; `staffApi.ts`'s `listCandidates` and `getCandidateFull` do the same client-side reduction for the Candidates table and staff's CandidateForm. `step_report_history` (the full per-attempt log behind the Session Log/Report timeline) is untouched -- this only changes what counts as "current."

### Network Check simplified to evidence-only

The network check isn't a step with a qualitative outcome to report -- it's just proof the candidate logged in. Dropped the outcome radios and comment field from `NetworkCheck.tsx`; it's now a single required evidence upload (`EvidenceList` gained a `maxFiles` prop, defaulting to the existing 5-file cap; the network check passes `maxFiles={1}`), and "Continue" unlocks as soon as one file is attached. Saving still writes to the same `step_reports` row as before (a fixed `completed` outcome, empty comment), so nothing about the underlying data model changed -- only what the candidate is asked for.
