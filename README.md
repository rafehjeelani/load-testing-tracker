...
### Report: mutually exclusive completion tiles, a monotonic funnel, and Session Durations

- **"Completed All Steps" and "Completed With Issues" (renamed from "Unable to Complete", which read as a candidate never finishing at all -- misleading, since it actually meant "Completed with issues on at least one step") no longer double-count candidates.** "Completed All Steps" used to mean "has *some* outcome for every step," which included candidates who'd reported "Completed with issues" somewhere -- so a candidate could land in both tiles, and the two could sum to more than the invited count. It now requires every step to be "Completed" specifically (not "Completed with issues"), so the two tiles partition candidates the way the labels suggest. The same fix and rename apply to the moderator Dashboard's matching tile.
- **The Candidate Funnel can no longer go back up.** Each step's raw count is independent ("how many candidates currently have *this* step done"), and now that the wizard lets candidates jump around via the dropdown or Skip a step, a later step can legitimately have more current-attempt completions than an earlier one it skipped past -- which a funnel chart should never show. Each stage's count is now clamped to `Math.min(itself, the previous stage's count)`, so the bars only ever narrow.
- **New "Session Durations" section**: three total-duration tiles (Primary, Screen, Secondary) plus a per-candidate table. Primary = Face Captured → Session Completed; Screen = Screen Shared → Session Completed; Secondary = Onboarding Completed (Orientation check Submitted) → Session Completed (the event when secondary recording starts). If an anchor step was re-answered after a disconnection, only the active segments are summed (the disconnected gap is excluded); if it wasn't re-answered, the duration includes that downtime and the cell is flagged (hover for why). If "Session Completed" itself was never reported, the end time falls back to the last recorded event for that candidate. Matches to the minute against an independent Python implementation run over the same data. Built from `stepHistory`/`issues`, already fetched for the Session Timeline -- no new queries or schema changes.

### Standard step list, and reference screenshots guiding candidates on evidence

- **Every new test now starts with the standard 11-step list** (`STANDARD_STEPS` in `staffApi.ts`, matching the order in the team's Screenshot Evidence Guide), instead of starting with zero steps. "Copy Steps From" on the Create Test form still works as an override for a test that deliberately wants a different test's steps; picking neither now falls back to the standard list rather than an empty one.
- **Reference screenshots on every step**: the candidate wizard now shows a "Reference: what to capture" panel -- the exact guidance text from the Screenshot Evidence Guide (e.g. "Completed speed-test page with download and upload speeds visible") above a full-width image, large enough to read without clicking to enlarge or downloading it. `src/lib/referenceScreenshots.ts` keys these by the step's exact name, matching `STANDARD_STEPS`; a step that's been renamed or customized just won't show a reference image, which is expected. The wizard's intro text also now points candidates to log errors, reconnecting screens, or long delays under Disconnection instead of uploading them as step evidence. The Network Check gate itself doesn't show a reference panel -- it's a fixed evidence-upload step, not part of the reference-matching flow. Every source image was cropped to remove the browser tab bar and OS taskbar, since those became more prominent clutter at the larger display size. One source image (Secondary Device Connected) also had a Teams notification exposing real email addresses and what looked like a plaintext password -- redacted before cropping, since this page is public and unauthenticated.

### Which stream disconnected, a reason prompt before overwriting saved data, and per-stream durations

- **Disconnection now records which stream(s) were affected.** The "Report an Issue" modal (`src/components/IssuesSection.tsx`, shared by the candidate wizard and staff editing) adds a required multi-select -- Primary disconnected / Screen disconnected / Secondary disconnected -- alongside the existing comment and evidence requirements; submitting needs at least one checked. Stored as `issues.disconnected_streams text[]` (migration `0019_disconnected_streams.sql`), validated server-side in `rpc_add_issue` the same way comment/evidence already were. Already-logged disconnections show which streams they affected as badges, both in the Issues & Disconnections list and the Session Log.
- **Changing an already-answered step's outcome or saved time now asks why first**, instead of applying silently (or, for unchecking, a plain browser confirm). A new `EditReasonModal` (shared by the candidate `StepRow` and staff `StaffStepRow`) offers two reasons: "Disconnection happened -- log it" (discards the pending change and opens the Disconnection form instead, so the real cause gets recorded properly) or "Just correcting a mistake" (applies the change as before). This only gates *changing* existing data -- picking an outcome for the first time on an untouched step still saves immediately, no prompt.
- **Report page duration math now excludes downtime only from the stream(s) actually disconnected**, instead of subtracting every disconnection from all three (Primary/Screen/Secondary) uniformly. An issue logged with no streams recorded (everything from before this migration) still counts against all three, matching the old behavior -- only newly-logged, stream-tagged disconnections narrow it down. The issues CSV export also gained a "Disconnected Streams" column.
